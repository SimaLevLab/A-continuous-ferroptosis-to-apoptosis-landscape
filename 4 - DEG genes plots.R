#create the genes bar graphs (Figure 2F, 3G, 4G)
...
#-------------------------------------------------------------------------------------------
# Ferroptosis related JB2 synergistic genes (Fig. 2F)
#-------------------------------------------------------------------------------------------

#1. Ferroptosis related genes, based on the Venn diagrams:
pick <- c(SynGenes_both, SynGenes_both_down, SynGenes_468_6h_up, SynGenes_468_6h_down) #create these objects in the "3 - DEG analysis (Fig. 2).R" script
...
#add GO annotations, using enrichR (`EnrichrFun` function is defined in the "3 - DEG analysis (Fig. 2).R" script)
...
ggsave("../results/Figure 2F.tiff", dpi = 200, height = 8, width = 20)

#-------------------------------------------------------------------------------------------
# Lysosomal genes in JB2 (Fig. 3G)
#-------------------------------------------------------------------------------------------
...
p2 <- ggplot(thisFig %>% filter(group %in% faceting_groups), aes(x = value, y = genes, fill = group)) +
  ...
  geom_text(aes(label = pval), size = 5) +
  geom_point(data = gene_sorter %>% filter(group %in% faceting_groups) %>% filter(!is.na(MITF)), 
             x = -6.5, color = "red", size = 3, show.legend = FALSE) +
  geom_point(data = gene_sorter %>% filter(group %in% faceting_groups) %>% filter(!is.na(TFEB)), 
             x = -7, color = "blue", size = 3, show.legend = FALSE) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_grid(group~cell_line, scales = "free_y", space = "free", labeller = label_wrap_gen(width = 20)) +
  labs(x = "Fold change vs. DMSO (Log2)", y = "", fill = "Function") +
  coord_cartesian(clip = "off") +
...
ggsave("../results/Figure 3G.tiff", dpi = 200, height = 12, width = 15)

#-------------------------------------------------------------------------------------------
# ATF4-only targets (Figure 4G)
#-------------------------------------------------------------------------------------------

pick <- DEG_combined %>% filter(JB2_24h_fdr == 1) %>% pull(genes)
ATF4 <- genesets[["ATF4 only"]] %>% as_tibble() %>% 
...
ggarrange(p1, p2, align = "hv")
ggsave("../results/Figure 4G.tiff", dpi = 200, height = 12, width = 15)