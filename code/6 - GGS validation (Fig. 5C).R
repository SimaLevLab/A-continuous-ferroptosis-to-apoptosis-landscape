#This script will validate the Gradient Gene Set, as shown in figure 5C

#NOTE: 
#Run-time of this script is ~2 hours, due to the need to analyze 1000 iterations of random genesets.
#For fast reproduction of the code, we offer a saved object replacing a "for" loop (see later in the code)
...
Contour_plot <- function(geneset, plot = TRUE, title = "test") {
  ...
  if(plot) {
    ...
    p1
    ggsave(paste0("../results/Figure 5C - Contour plot (UMAP method) for ", title, ".tiff"), dpi = 300, height = 5, width = 7)
...
#and calculate the optimized AUC of classification using these random genes.
#run time of this for loop: ~6.5 sec/iteration, ~1.8 hours for 1000 iterations.
#option: To save time, run the following line in order to load the "random_results" object, created by this for loop:
#load("../data/Response signature results for random genesets (1000 permutations).Rdata")
set.seed(123)
iterations <- 1000
set_size <- 306
imputation_cutoff <- 12
random_results <- list()

for(n in 1:iterations) {
  Random_set <- Datasets_combined %>% slice_sample(n = set_size) %>% pull(ID)
  print(paste0("Iteration: ", n, " / ", iterations, ", genes: ", paste0(head(Random_set, n = 10), collapse = " ")))
  title = str_c("Random_", n)
  random_results[[n]] <- Contour_plot(geneset = Random_set, plot = FALSE, title = title) %>% 
    rename_with(.cols = value, .fn = ~title)
}

#after creating or loading the "random_results" object, continue from here, to calculate the p-value of the AUC of the GGS.
#for each iteration, the best AUC is selected (optimized by k-nearest neighbors)
random_results_tib <- purrr::reduce(random_results, left_join, by = "parameter") 
random_AUCs <- random_results_tib %>% 
  gather(-parameter, key = "set", value = "value") %>% 
  filter(str_detect(parameter, "auc")) %>% 
  group_by(set) %>% 
  slice_max(value, n = 1, with_ties = FALSE)

#how many random AUCs are higher than the AUC of the GGS?
GGS_AUC <- CP_for_GGS %>% 
  filter(str_detect(parameter, "auc")) %>% 
  slice_max(SM100, n = 1, with_ties = FALSE) %>% 
  pull(SM100)
GGS_pvalue <- sum(GGS_AUC < random_AUCs$value) / iterations
