...
#This script will create the ferroptosis vs. apoptosis specific biomarkers set, as shown in figure 2 and S2
...
#and 26 apoptosis inducers (AINs) datasets.
...
#run time of the following block of code: ~90 minutes
##option: To save time, run the following line in order to load the object created by this block ("Datasets_derived_sig"):
...
#1D plot of the ferroptosis vs. apoptosis scores from the above analysis (Fig. S2D)
set.seed(123)
...
ggsave("../results/Figure S2D - Dataset derived set visualized in 1D.tiff", dpi = 300, height = 2.5, width = 9)
...
ggsave("../results/Figure S2E (left) - AUCs of datasets derived biomarkers.tiff", dpi = 300, height = 5, width = 5)
...
#download the following GMT file here: https://data.broadinstitute.org/gsea-msigdb/msigdb/release/7.2/
hallmark <- gmt2list("../data/h.all.v7.2.symbols.gmt") #will be used to score the "Hallmark_apoptosis" set
...
signatures <- list(#our sets - the gradient derived and datasets derived biomarkers
                   ...
                   "Hallmark_Apoptosis" = unique(hallmark[["HALLMARK_APOPTOSIS"]]))
...
#plot a scatterplot of the gradient-derived and datasets-derived scores for all datasets (Fig. S2F-G)
...
ggsave("../results/Figure S2F - Contour plot of biomarkers in both sets (showing datasets).tiff", dpi = 300, height = 6, width = 7)
...
ggsave("../results/Figure S2G - Contour plot of biomarkers in both sets (showing JB combinations).tiff", 
       dpi = 300, height = 6, width = 7)

#box-plots of the signatures (Figure 2D)
thisFig <- ssgsea_result %>% 
  left_join(AUCs_all, by = c("pathway", "method")) %>% 
  filter(type != "JB combinations") %>% 
...
ggsave("../results/Figure 2D - ssGSEA scores of the biomarker sets for datasets.tiff", dpi = 300, height = 5, width = 8)
...
ggsave("../results/Figure 2J - Final biomarkers (25) enrichment in all datasets (aggregated).tiff", dpi = 300, height = 6, width = 2.5)

#plot the AUCs of the gradient-derived set by using different geneset size (10-26 genes) (Figure S2E, right)
set.seed(123)
take <- c("^logFC")
cutoffs <- seq(10, 26, 1)
...
ggsave("../results/Figure S2E (right) - AUCs of gradient derived biomarkers.tiff", dpi = 300, height = 5, width = 5)

#venn diagram depicting the inverse gradient idea (Figure S2C)
...
tiff(filename = "../results/Figure S2C - Euler plot for inverse gradient genes.tiff", 
     width = 600, height = 600)
plot(euler(InvGrad_Venn), quantities = list(cex = 2), lty = 1, lwd = 2)
dev.off()

#venn diagram of all the biomarkers (Fig 2E)
...
venn_partitions <- this_venn_partitions %>% 
  rowwise() %>% 
  mutate(..values.. = paste0(..values.., collapse = " ")) #actual Venn shown in Fig. 2E was drawn in powerpoint based on this object
